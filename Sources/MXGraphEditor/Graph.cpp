...
Graph::Graph(const std::string &materialFilename,
             ...
             const mx::FilePathVec &libraryFolders, int viewWidth,
             int viewHeight)
    : _materialFilename(materialFilename), _searchPath(searchPath),
      _libraryFolders(libraryFolders), _initial(false), _delete(false),
      _fileDialogSave(FileDialog::EnterNewFilename), _isNodeGraph(false),
      _graphTotalSize(0), _popup(false), _shaderPopup(false), _searchNodeId(-1),
      _addNewNode(false), _ctrlClick(false), _isCut(false), _autoLayout(false),
      _frameCount(INT_MIN), _fontScale(1.0f), _saveNodePositions(true) {
  ...
  _renderer =
      std::make_shared<RenderView>(_graphDoc, meshFilename, envRadianceFilename,
...
void Graph::loadStandardLibraries() {
  ...
  try {
    ...
    if (_xincludeFiles.empty()) {
      std::cerr
          << "Could not find standard data libraries on the given search path: "
  ...
  try {
    if (!filename.isEmpty()) {
      ...
      if (!doc->validate(&message)) {
        std::cerr << "*** Validation warnings for " << filename.asString()
                  << " ***" << std::endl;
...
ed::PinId Graph::getOutputPin(UiNodePtr node, UiNodePtr upNode,
                              UiPinPtr input) {
  if (upNode->getNodeGraph() != nullptr) {
    // For nodegraph need to get the correct ouput pin accorinding to the names
    // of the output nodes
    mx::OutputPtr output =
        input->_pinNode->getNode()
...
void Graph::findYSpacing(float startY) {
  ...
  // For each level find the average y position of the previous level to use as
  // a spacing guide
  int i = 0;
  for (std::pair<int, std::vector<UiNodePtr>> levelChunk : _levelMap) {
    if (_levelMap[i].size() > 0) {
      if (_levelMap[i][0]->_level > 0) {
        ...
        // caculate the starting position to be above the previous level's
        // center so that it is evenly spaced on either side of the center
        float startingPos =
...
ImVec2 Graph::layoutPosition(UiNodePtr layoutNode, ImVec2 startingPos,
                             bool initialLayout, int level) {
  if (checkPosition(layoutNode) && !_autoLayout) {
    for (UiNodePtr node : _graphNodes) {
      // Since nodegraph nodes do not have MaterialX info they are placed based
      // on their connected node
      if (node->getNodeGraph() != nullptr) {
        ...
        if (outputCon.size() > 0) {
          ...
          ed::SetNodePosition(node->getId(),
                              ImVec2(outputPos.x - 400, outputPos.y));
        ...
        if (pins.size() != 0 && layoutNode->getInput() == nullptr) {
          for (size_t i = 0; i < pins.size(); i++) {
            ...
            if (nextNode) {
              ...
              // Call layout position on upstream node with newPos to the left
              // of current node
              layoutPosition(nextNode, ImVec2(newPos.x, startingPos.y),
                             initialLayout, layoutNode->_level + 1);
            }
          }
        }
      } else {
  ...
  if (_levelMap.begin() != _levelMap.end()) {
    ...
    for (UiNodePtr uiNode : _graphNodes) {
      if (uiNode->getOutputConnections().size() == 0 &&
        ...
        startingPos.y += 23;
      } else if (uiNode->getOutputConnections().size() == 0 &&
...
void Graph::setPinColor() {
  ...
  _pinColor.insert(std::make_pair(mx::SURFACE_SHADER_TYPE_STRING,
                                  ImColor(150, 255, 255, 255)));
  _pinColor.insert(
      std::make_pair(mx::MATERIAL_TYPE_STRING, ImColor(255, 255, 255, 255)));
  _pinColor.insert(std::make_pair(mx::DISPLACEMENT_SHADER_TYPE_STRING,
                                  ImColor(155, 50, 100, 255)));
  _pinColor.insert(std::make_pair(mx::VOLUME_SHADER_TYPE_STRING,
                                  ImColor(155, 250, 100, 255)));
  _pinColor.insert(std::make_pair(mx::LIGHT_SHADER_TYPE_STRING,
                                  ImColor(100, 150, 100, 255)));
  _pinColor.insert(std::make_pair("none", ImColor(140, 70, 70, 255)));
  _pinColor.insert(
      std::make_pair(mx::MULTI_OUTPUT_TYPE_STRING, ImColor(70, 70, 70, 255)));
...
void Graph::selectMaterial(UiNodePtr uiNode) {
  ...
  std::vector<mx::TypedElementPtr> elems =
      mx::findRenderableElements(_graphDoc);
...
void Graph::setRenderMaterial(UiNodePtr node) {
  ...
  // Traverse downstream looking for the first renderable element.
    ...
    if (mtlxNode) {
      ...
      if (parent->isA<mx::NodeGraph>()) {
        // There is no logic to support traversing from inside a functional
        // graph to it's instance and hence downstream so skip this from
        // consideration. The closest approach would be to "flatten" all
        // definitions to compound graphs.
...
void Graph::updateMaterials(mx::InputPtr input /* = nullptr */,
...
void Graph::setConstant(UiNodePtr node, mx::InputPtr &input,
  ...
  if (input->getType() == "float") {
    ...
    if (val && val->isA<mx::Vector2>()) {
      mx::Vector2 prev = val->asA<mx::Vector2>(),
                  temp = val->asA<mx::Vector2>();
    ...
    if (val && val->isA<mx::Vector3>()) {
      mx::Vector3 prev = val->asA<mx::Vector3>(),
                  temp = val->asA<mx::Vector3>();
    ...
    if (val && val->isA<mx::Vector4>()) {
      mx::Vector4 prev = val->asA<mx::Vector4>(),
                  temp = val->asA<mx::Vector4>();
    ...
    if (val && val->isA<std::string>()) {
      std::string prev = val->asA<std::string>(),
                  temp = val->asA<std::string>();
    ...
    if (val && val->isA<std::string>()) {
      std::string temp = val->asA<std::string>(),
                  prev = val->asA<std::string>();
      ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.15f, .15f, .15f, 1.0f));
      ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                            ImVec4(.2f, .4f, .6f, 1.0f));
...
void Graph::setUiNodeInfo(UiNodePtr node, const std::string &type,
  ...
  if (node->getNodeGraph()) {
    ...
    for (mx::OutputPtr out : outputs) {
      UiPinPtr outPin = std::make_shared<UiPin>(
          _graphTotalSize, &*out->getName().begin(), out->getType(), node,
    ...
    for (mx::InputPtr input : node->getNodeGraph()->getInputs()) {
      UiPinPtr inPin = std::make_shared<UiPin>(
          _graphTotalSize, &*input->getName().begin(), input->getType(), node,
          ax::NodeEditor::PinKind::Input, input, nullptr);
    ...
    if (node->getNode()) {
      ...
      if (nodeDef) {
        for (mx::InputPtr input : nodeDef->getActiveInputs()) {
          ...
          UiPinPtr inPin = std::make_shared<UiPin>(
              _graphTotalSize, &*input->getName().begin(), input->getType(),
        ...
        for (mx::OutputPtr output : nodeDef->getActiveOutputs()) {
          ...
          UiPinPtr outPin = std::make_shared<UiPin>(
              _graphTotalSize, &*output->getName().begin(), output->getType(),
      ...
      UiPinPtr inPin = std::make_shared<UiPin>(
          _graphTotalSize, &*("Value"), node->getInput()->getType(), node,
      ...
      UiPinPtr inPin = std::make_shared<UiPin>(
          _graphTotalSize, &*("input"), node->getOutput()->getType(), node,
    ...
    if (node->getInput() || node->getOutput()) {
      UiPinPtr outPin = std::make_shared<UiPin>(
          _graphTotalSize, &*("output"), type, node,
          ax::NodeEditor::PinKind::Output, nullptr, nullptr);
      ++_graphTotalSize;
      node->outputPins.push_back(outPin);
      _currPins.push_back(outPin);
  ...
  for (const auto &group : NODE_GROUP_ORDER) {
    ...
    if (it != groupToNodeDef.end()) {
      ...
      for (const auto &nodeDef : groupNodeDefs) {
        _nodesToAdd.emplace_back(nodeDef->getName(), nodeDef->getType(),
                                 nodeDef->getNodeString(), group);
      }
    }
  }
          ...
          if (downNode > 0 && upNode > 0 &&
            ...
            if (connectingInput) {
              if ((upNode >= 0) && (downNode >= 0)) {
                UiEdge newEdge = UiEdge(_graphNodes[upNode],
                                        _graphNodes[downNode], connectingInput);
                if (!edgeExists(newEdge)) {
                  _graphNodes[downNode]->edges.push_back(newEdge);
                  _graphNodes[downNode]->setInputNodeNum(1);
          ...
          if (upstreamNode) {
            ...
            for (mx::InputPtr input : ins) {
              ...
              if (input->hasInterfaceName()) {
                ...
                if (newUp >= 0) {
                  mx::InputPtr inputP = std::make_shared<mx::Input>(
                      downstreamElem, input->getName());
                  UiEdge newEdge =
                      UiEdge(_graphNodes[newUp], _graphNodes[upNode], input);
                  if (!edgeExists(newEdge)) {
    ...
    for (mx::ElementPtr elem : children) {
      ...
      if (node) {
        ...
        for (mx::InputPtr input : inputs) {
          ...
          if (upNode) {
            ...
            if ((upNum >= 0) && (downNode >= 0)) {
              ...
              UiEdge newEdge =
                  UiEdge(_graphNodes[upNum], _graphNodes[downNode], input);
              if (!edgeExists(newEdge)) {
                _graphNodes[downNode]->edges.push_back(newEdge);
                _graphNodes[downNode]->setInputNodeNum(1);
            ...
            int upNum =
                findNode(input->getInterfaceInput()->getName(), "input");
            int downNode = findNode(node->getName(), "node");
            if ((upNum >= 0) && (downNode >= 0)) {
        ...
        if (upNode) {
          ...
          UiEdge newEdge =
              UiEdge(_graphNodes[upNum], _graphNodes[downNode], nullptr);
          if (!edgeExists(newEdge)) {
            _graphNodes[downNode]->edges.push_back(newEdge);
            _graphNodes[downNode]->setInputNodeNum(1);
            _graphNodes[upNum]->setOutputConnection(_graphNodes[downNode]);
...
void Graph::createEdge(UiNodePtr upNode, UiNodePtr downNode,
  ...
  for (std::map<UiNodePtr, UiNodePtr>::iterator iter = _copiedNodes.begin();
    ...
    for (UiPinPtr pin : origNode->inputPins) {
      if (origNode->getConnectedNode(pin->_name) && !_ctrlClick) {
        // If original node is connected check if connect node is in copied
        // nodes
        if (_copiedNodes.find(origNode->getConnectedNode(pin->_name)) !=
            _copiedNodes.end()) {
          // Set copy node connected to the value at this key
          createEdge(_copiedNodes[origNode->getConnectedNode(pin->_name)],
                     copyNode, copyNode->inputPins[count]->_input);
          UiNodePtr upNode =
          ...
          if (copyNode->getNode() || copyNode->getNodeGraph()) {
            ...
            if (copyNode->getNode() && copyNode->getNode()->getType() ==
                                           mx::SURFACE_SHADER_TYPE_STRING) {
              if (upNode->getOutput()) {
                ...
                copyNode->inputPins[count]->_input->setInterfaceName(
                    upNode->getName());
              } else {
                if (upNode->getNodeGraph()) {
                  ed::PinId outputId = getOutputPin(copyNode, upNode,
                                                    copyNode->inputPins[count]);
                  for (UiPinPtr outPin : upNode->outputPins) {
                    if (outPin->_pinId == outputId) {
                      mx::OutputPtr outputs =
                          upNode->getNodeGraph()->getOutput(outPin->_name);
                      copyNode->inputPins[count]->_input->setConnectedOutput(
                          outputs);
                    }
                  }
            ...
            copyNode->inputPins[count]->_input->removeAttribute(
...
void Graph::addNode(const std::string &category, const std::string &name,
  ...
  if (node) {
    ...
    // Add inputs to UiNode as pins so that we can later add them to the node if
    // necessary
    auto newNode =
...
UiPinPtr Graph::getPin(ed::PinId pinId) {
  ...
  UiPinPtr nullPin = std::make_shared<UiPin>(-10000, "nullPin", "null", nullptr,
...
void Graph::drawPinIcon(std::string type, bool connected, int alpha) {
  ...
  ax::Widgets::Icon(ImVec2(24, 24), iconType, connected, color,
...
void Graph::drawOutputPins(UiNodePtr node,
  ...
  for (UiPinPtr pin : node->outputPins) {
    ...
    if (!_pinFilterType.empty()) {
      drawPinIcon(pin->_type, connected,
      ...
      if (node->getNode() != nullptr) {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(
            ...
            ImGui::GetCursorScreenPos() +
                ImVec2(ed::GetNodeSize(node->getId()).x - 9.f,
        ...
        for (UiPinPtr pin : node->inputPins) {
          ...
          if (node->_showAllInputs ||
              (pin->getConnected() || node->getNode()->getInput(pin->_name))) {
            drawInputPin(pin);
        ...
        for (UiPinPtr pin : node->inputPins) {
          ...
          if (upUiNode) {
            if (upUiNode->outputPins.size()) {
              std::string outString = pin->_output
                                          ? pin->_output->getOutputString()
                                          : mx::EMPTY_STRING;
              size_t pinIndex = 0;
              if (!outString.empty()) {
                for (size_t i = 0; i < upUiNode->outputPins.size(); i++) {
...
void Graph::addLink(ed::PinId startPinId, ed::PinId endPinId) {
  // Prefer to assume left to right - start is an output, end is an input; swap
  // if inaccurate
  ...
  if (!typesMatch) {
    ed::RejectNewItem();
    showLabel("Invalid connection due to mismatched types",
  ...
  if (kindsMatch) {
    ed::RejectNewItem();
    showLabel("Invalid connection due to same input/output kind",
              ImColor(50, 50, 50, 255));
    return;
  }
  ...
  if (uiDownNode->getInput() && uiUpNode->getOutput()) {
    ed::RejectNewItem();
    showLabel("Direct connections between inputs and outputs is invalid",
              ImColor(50, 50, 50, 255));
    return;
  }
  ...
  if (uiDownNode->getInput() && _isNodeGraph) {
    ed::RejectNewItem();
    showLabel("Cannot connect to inputs inside of graph",
              ImColor(50, 50, 50, 255));
    return;
    ...
    if (!impl) {
      ed::RejectNewItem();
      showLabel("Invalid Connection: Node does not have an implementation",
                ImColor(50, 50, 50, 255));
      return;
    }
  }
  ...
  if (ed::AcceptNewItem()) {
    ...
    if (inputPin->_connected) {
      for (auto linksItr = _currLinks.begin(); linksItr != _currLinks.end();
           linksItr++) {
    ...
    if (uiDownNode->getNode() || uiDownNode->getNodeGraph()) {
      ...
      for (UiPinPtr pin : uiDownNode->inputPins) {
        if (pin->_pinId == inputPinId) {
          ...
          // Update value to be empty
          if (uiDownNode->getNode() && uiDownNode->getNode()->getType() ==
                                           mx::SURFACE_SHADER_TYPE_STRING) {
              ...
              if (uiUpNode->getNodeGraph() != nullptr) {
                for (UiPinPtr outPin : uiUpNode->outputPins) {
                  ...
                  if (outPin->_pinId == outputPinId) {
                    mx::OutputPtr outputs =
                        uiUpNode->getNodeGraph()->getOutput(outPin->_name);
            ...
            if (uiUpNode->getInput()) {
              ...
              if (uiUpNode->getNode()) {
                ...
                bool isMultiOutput =
                    upstreamNodeDef ? upstreamNodeDef->getOutputs().size() > 1
                                    : false;
                if (!isMultiOutput) {
                  pin->_input->setConnectedNode(uiUpNode->getNode());
                } else {
                  for (UiPinPtr outPin : _graphNodes[upNode]->outputPins) {
                    ...
                    if (outPin->_pinId == outputPinId) {
                      mx::OutputPtr outputs =
                          uiUpNode->getNode()->getOutput(outPin->_name);
                      if (!outputs) {
                        outputs = uiUpNode->getNode()->addOutput(
                            outPin->_name, pin->_input->getType());
                      }
                      pin->_input->setConnectedOutput(outputs);
                    }
                  }
...
void Graph::removeEdge(int downNode, int upNode, UiPinPtr pin) {
  ...
  if (num != -1) {
    if (_graphNodes[downNode]->edges.size() == 1) {
      _graphNodes[downNode]->edges.erase(_graphNodes[downNode]->edges.begin() +
                                         0);
      ...
      if ((int)pin->_pinId.Get() == endAttr) {
        ...
        if (_graphNodes[upNode]->getInput()) {
          _graphNodes[downNode]
              ->getNodeGraph()
              ->getInput(pin->_name)
              ->removeAttribute(mx::ValueElement::INTERFACE_NAME_ATTRIBUTE);
          setDefaults(_graphNodes[upNode]->getInput());
        }
        for (UiPinPtr connect : pin->_connections) {
          pin->deleteConnection(connect);
    ...
    if (node->getNodeGraph()) {
      if (node->inputPins.size() != node->getNodeGraph()->getInputs().size()) {
        for (mx::InputPtr input : node->getNodeGraph()->getInputs()) {
          ...
          auto result =
              std::find_if(node->inputPins.begin(), node->inputPins.end(),
      ...
      if (node->outputPins.size() !=
        ...
        for (mx::OutputPtr output : node->getNodeGraph()->getOutputs()) {
          ...
          auto result =
              std::find_if(node->outputPins.begin(), node->outputPins.end(),
                           [name](UiPinPtr x) { return x->_name == name; });
          if (result == node->outputPins.end()) {
            UiPinPtr outPin = std::make_shared<UiPin>(
                ++_graphTotalSize, &*output->getName().begin(),
                output->getType(), node, ax::NodeEditor::PinKind::Output,
                nullptr, nullptr);
            ++_graphTotalSize;
            node->outputPins.push_back(outPin);
            _currPins.push_back(outPin);
...
void Graph::graphButtons() {
  ...
  // RenderView window
  ImVec2 wsize = ImVec2((float)_renderer->getViewWidth(),
                        (float)_renderer->getViewHeight());
  ...
  if (_renderer) {
    ...
    ImGui::Image((ImTextureID)my_image_texture, screenSize, ImVec2(0, 1),
                 ImVec2(1, 0));
  }
  ImGui::Separator();
...
void Graph::propertyEditor() {
  ...
  if (_currUiNode) {
    ...
    if (_currUiNode->getNode()) {
      if (temp != original) {
        std::string name =
            _currUiNode->getNode()->getParent()->createValidChildName(temp);
          ...
          if (nodes->getInput() == nullptr) {
            for (mx::InputPtr input : nodes->getNode()->getActiveInputs()) {
              if (input->getConnectedNode() == _currUiNode->getNode()) {
                ...
                nodes->getNode()->setConnectedNode(input->getName(),
                                                   _currUiNode->getNode());
              }
            }
          }
      ...
      if (temp != original) {
        std::string name =
            _currUiNode->getInput()->getParent()->createValidChildName(temp);
        std::vector<UiNodePtr> downstreamNodes =
            _currUiNode->getOutputConnections();
        for (UiNodePtr nodes : downstreamNodes) {
          if (nodes->getInput() == nullptr) {
      ...
      if (temp != original) {
        std::string name =
            _currUiNode->getOutput()->getParent()->createValidChildName(temp);
      ...
      if (temp != original) {
        std::string name =
            _currUiNode->getNodeGraph()->getParent()->createValidChildName(
                temp);
    ...
    if (_currUiNode->getNode()) {
      ...
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
        ImGui::SetTooltip(
            "%s", _currUiNode->getNode()->getNodeDef()->getDocString().c_str());
      }
      ...
      if (count) {
        ...
        bool haveTable =
            ImGui::BeginTable("inputs_node_table", 2, tableFlags, tableSize);
        if (haveTable) {
          ImGui::SetWindowFontScale(_fontScale);
          for (UiPinPtr input : _currUiNode->inputPins) {
            if (_currUiNode->_showAllInputs ||
              ...
              mx::getUIProperties(input->_input, mx::EMPTY_STRING,
                                  uiProperties);
              ...
              mx::InputPtr tempInt =
                  _currUiNode->getNode()->getNodeDef()->getActiveInput(
                      input->_input->getName());
      ...
      if (count) {
        bool haveTable = ImGui::BeginTable(
            "inputs_input_table", 2, tableFlags,
            ImVec2(0.0f,
                   TEXT_BASE_HEIGHT * std::min(SCROLL_LINE_COUNT, count)));
        if (haveTable) {
          ImGui::SetWindowFontScale(_fontScale);
      ...
      if (count) {
        bool haveTable = ImGui::BeginTable(
            "inputs_nodegraph_table", 2, tableFlags,
            ImVec2(0.0f,
                   TEXT_BASE_HEIGHT * std::min(SCROLL_LINE_COUNT, count)));
        if (haveTable) {
          ImGui::SetWindowFontScale(_fontScale);
          for (UiPinPtr input : inputs) {
            if (_currUiNode->_showAllInputs ||
              ...
              docString += _currUiNode->getNodeGraph()
                               ->getActiveInput(input->_input->getName())
                               ->getDocString();
...
void Graph::showHelp() const {
  ...
  if (ImGui::CollapsingHeader("Property Editor")) {
    ...
    ImGui::BulletText(
        "DBL_CLICK or CTRL+CLICK LEFT-MOUSE on entry field to input values.");
    ImGui::Separator();
    ImGui::BulletText("\"Show all inputs\" Will toggle between showing all "
...
void Graph::addNodePopup(bool cursor) {
  ...
  if (ImGui::BeginPopup("add node")) {
    ...
    for (auto node : _nodesToAdd) {
      ...
      if (subs.size() > 0) {
        ImGui::SetNextWindowSizeConstraints(ImVec2(250.0f, 300.0f),
                                            ImVec2(-1.0f, 500.0f));
        ...
        if (str.find(subs) != std::string::npos) {
          if (ImGui::MenuItem(getUserNodeDefName(nodeName).c_str()) ||
            ...
            addNode(node.getCategory(), getUserNodeDefName(nodeName),
                    node.getType());
            _addNewNode = true;
        ...
        if (ImGui::BeginMenu(node.getGroup().c_str())) {
          ...
          if (name.compare(0, prefix.size(), prefix) == 0 &&
              name.compare(prefix.size(), std::string::npos,
                           node.getCategory()) == 0) {
            if (ImGui::MenuItem(getUserNodeDefName(name).c_str()) ||
                (ImGui::IsItemFocused() &&
                 ImGui::IsKeyPressed(ImGuiKey_Enter))) {
              addNode(node.getCategory(), getUserNodeDefName(name),
                      node.getType());
              _addNewNode = true;
            }
          } else {
...
void Graph::shaderPopup() {
  if (_renderer->getMaterialCompilation()) {
    ImGui::SetNextWindowPos(ImVec2((float)_renderer->getViewWidth() - 135,
  ...
  ed::Begin("My Editor");
  {
    ...
    if (selectedNodes.size() > 0) {
      ...
      if (graphPos > -1) {
        ...
        if (!_prevUiNode ||
            (_prevUiNode->getName() != _graphNodes[graphPos]->getName())) {
    ...
    // Address copy information if applicable and relink graph if a new node has
    // been added
    if (_addNewNode) {
      copyInputs();
      linkGraph();
    ...
    if (_initial || _autoLayout) {
      ...
      // Start layout with output or material nodes since layout algorithm works
      // right to left
      ...
      // If there are no output or material nodes but the nodes have position
      // layout each individual node
    ...
    if (ImGui::IsKeyReleased(ImGuiKey_U) && (!ImGui::IsPopupOpen("add node")) &&
        (!ImGui::IsPopupOpen("search")) && !_fileDialogSave.isOpened()) {
      upNodeGraph();
    }

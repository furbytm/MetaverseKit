...
bool MslMaterial::loadSource(const FilePath &vertexShaderFile,
...
bool MslMaterial::generateShader(GenContext &context) {
  ...
  _hasTransparency =
      isTransparentSurface(_elem, context.getShaderGenerator().getTarget());
...
void MslMaterial::prepareUsedResources(CameraPtr cam,
  ...
  _glProgram->prepareUsedResources(MTL(renderCmdEncoder), cam, geometryHandler,
...
void MslMaterial::bindImages(ImageHandlerPtr imageHandler,
  ...
  // Texture and Samplers being bound to the right texture and sampler in
  // MslPipelineStateObject automatically.
}

ImagePtr
MslMaterial::bindImage(const FilePath &filePath, const std::string &uniformName,
                       ImageHandlerPtr imageHandler,
...
void MslMaterial::bindLighting(LightHandlerPtr lightHandler,
  ...
  if (shadowState.shadowMap &&
      _glProgram->hasUniform(TEXTURE_NAME(HW::SHADOW_MAP))) {
    ...
    // Bind the shadow map.
    _glProgram->bindTexture(imageHandler, TEXTURE_NAME(HW::SHADOW_MAP),
                            shadowState.shadowMap, samplingProperties);
    _glProgram->bindUniform(HW::SHADOW_MATRIX,
                            Value::createValue(shadowState.shadowMatrix));
  }

  // Bind ambient occlusion properties.
  if (shadowState.ambientOcclusionMap &&
      _glProgram->hasUniform(TEXTURE_NAME(HW::AMB_OCC_MAP))) {
    ImageSamplingProperties samplingProperties;
    samplingProperties.uaddressMode =
    ...
    _glProgram->bindUniform(
        HW::AMB_OCC_GAIN, Value::createValue(shadowState.ambientOcclusionGain));
  }
}
...
ShaderPort *MslMaterial::findUniform(const std::string &path) const {
  ...
  if (publicUniforms) {
    ...
    if (port &&
        !_glProgram->getUniformsList().count(publicUniforms->getInstance() +
                                             "." + port->getVariable())) {
      port = nullptr;
    }
  }
...
void MslMaterial::modifyUniform(const std::string &path, ConstValuePtr value,
  ...
  uniform->setValue(Value::createValueFromStrings(
      valueString, uniform->getType()->getName()));
  if (_doc) {

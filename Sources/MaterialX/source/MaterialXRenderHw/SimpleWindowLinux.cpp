...
bool SimpleWindow::initialize(const char *title, unsigned int width,
                              unsigned int height, void *applicationShell) {
  int n = 0;
  ...
  if (!applicationShell) {
    ...
    if (!initializedXServer) {
      batchShell = XtOpenApplication(&appContext, "__mx_dummy__app__", 0, 0, &n,
                                     0, 0, applicationShellWidgetClass, 0, 0);
  ...
  n++;
  Widget widget =
      XtCreatePopupShell(title, topLevelShellWidgetClass, shell, args, n);
  if (!widget) {
    _id = 0;
    return false;
  }
  ...
  _windowWrapper =
      WindowWrapper::create(widget, XtWindow(widget), XtDisplay(widget));

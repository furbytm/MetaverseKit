...
OslRenderer::OslRenderer(unsigned int width, unsigned int height,
                         Image::BaseType baseType)
    : ShaderRenderer(width, height, baseType), _useTestRender(true),
      _raysPerPixelLit(1), _raysPerPixelUnlit(1) {}
...
void OslRenderer::renderOSL(const FilePath &dirPath, const string &shaderName,
                            const string &outputName) {
  // If command options missing, skip testing.
  if (_oslTestRenderExecutable.isEmpty() || _oslIncludePath.isEmpty() ||
      _oslTestRenderSceneTemplateFile.isEmpty() ||
      _oslUtilityOSOPath.isEmpty()) {
    throw ExceptionRenderError("Command input arguments are missing");
  }

  static const StringSet RENDERABLE_TYPES = {
      "float",  "color",   "vector", "closure color",
      "color4", "vector2", "vector4"};
  static const StringSet REMAPPABLE_TYPES = {"color4", "vector2", "vector4"};

  // If the output type is not which can be supported for rendering then skip
  // testing.
  if (RENDERABLE_TYPES.count(_oslShaderOutputType) == 0) {
    throw ExceptionRenderError("Output type to render is not supported: " +
                               _oslShaderOutputType);
  }
  ...
  // Read in scene template and replace the applicable tokens to have a valid
  // ShaderGroup. Write to local file to use as input for rendering.
  std::ifstream sceneTemplateStream(_oslTestRenderSceneTemplateFile);
  string sceneTemplateString;
  sceneTemplateString.assign(
      std::istreambuf_iterator<char>(sceneTemplateStream),
  ...
  string outputShader =
      isColorClosure
          ? CLOSURE_PASSTHROUGH_SHADER_STRING
          : (isRemappable
                 ? CONSTANT_COLOR_SHADER_PREFIX_STRING + _oslShaderOutputType
                 : CONSTANT_COLOR_SHADER_STRING);

  // Perform token replacement
  const string ENVIRONMENT_SHADER_PARAMETER_OVERRIDES(
  ...
  const string INPUT_SHADER_PARAMETER_OVERRIDES(
  ...
  replacementMap[BACKGROUND_COLOR_STRING] =
      std::to_string(DEFAULT_SCREEN_COLOR_LIN_REC709[0]) + " " +
  ...
  if ((sceneString == sceneTemplateString) || sceneTemplateString.empty()) {
    throw ExceptionRenderError(
        "Scene template file: " + _oslTestRenderSceneTemplateFile.asString() +
  ...
  command += " -aa " + std::to_string(isColorClosure ? _raysPerPixelLit
                                                     : _raysPerPixelUnlit);
...
void OslRenderer::shadeOSL(const FilePath &dirPath, const string &shaderName,
                           const string &outputName) {
  ...
  // line. Note: This is currently hard-coded to a specific value. If testshade
  ...
  string successfulOutputSubString("Output " + outputName + " to " +
                                   outputFileName);
  while (std::getline(errorStream, line)) {
    if (!line.empty() && line.find(successfulOutputSubString) == string::npos) {
...
void OslRenderer::compileOSL(const FilePath &oslFilePath) {
  ...
  // Run the command and get back the result. If non-empty string throw
...
void OslRenderer::createProgram(ShaderPtr shader) {
  StageMap stages = {
      {Stage::PIXEL, shader->getStage(Stage::PIXEL).getSourceCode()}};
...
void OslRenderer::createProgram(const StageMap &stages) {
  ...
  bool haveCompiler =
      !_oslCompilerExecutable.isEmpty() && !_oslIncludePath.isEmpty();
  if (!haveCompiler) {
  ...
  // TODO: Seems testrender will crash currently when trying to convert to
  // "object" space. Thus we replace all instances of "object" with "world" to
  // avoid issues.
  StringMap spaceMap;
...
void OslRenderer::render() {
  if (_oslOutputFilePath.isEmpty()) {
    throw ExceptionRenderError(
        "OSL output file path string has not been specified");
  }
...
ImagePtr OslRenderer::captureImage(ImagePtr) {
  ...
  if (!_imageHandler || _oslOutputFileName.isEmpty()) {
    throw ExceptionRenderError("Failed to read image: " +
                               _oslOutputFileName.asString());
  }

  ImagePtr returnImage = _imageHandler->acquireImage(_oslOutputFileName);
  if (!returnImage) {
    throw ExceptionRenderError("Failed to save image to file: " +
                               _oslOutputFileName.asString());
  }

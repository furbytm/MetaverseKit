 ...
 * with std::vector parameters or return types, to automatically convert them to
 * / from JS arrays. It actually doesn't hurt to include this in every binding
 * file ;) Note that this only works for types that are known to Emscripten,
...
template <typename T> struct TypeID<std::vector<T>> {
  static constexpr TYPEID get() { return LightTypeID<val>::get(); }
};

template <typename T> struct TypeID<const std::vector<T>> {
  static constexpr TYPEID get() { return LightTypeID<val>::get(); }
};
...
template <> struct BindingType<std::vector<bool>> {
  using ValBinding = BindingType<val>;
  using WireType = ValBinding::WireType;
...
/**
 * Vectors of smart pointers need special treatment. The above generic
 * toWireType definition uses val::array(vec), which constructs a val::array
 * using val::array.call<void>("push", element). This leads to invalid (deleted)
 * smart pointers on the JS side, since the generated code constructs the smart
 * pointer object, pushes it into a JS array, and then deletes the smart pointer
 * object (i.e. sets the ref count to 0). Using val::array.set() doesn't suffer
 * from this issue.
 */
template <typename T> struct BindingType<std::vector<std::shared_ptr<T>>> {
  using ValBinding = BindingType<val>;
  using WireType = ValBinding::WireType;

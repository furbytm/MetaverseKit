 ...
 * with std::unordered_map parameters or return types, to automatically convert
 * them to / from JS objects. It actually doesn't hurt to include this in every
 * binding file ;) Note that this only works for types that are known to
 * Emscripten, i.e. primitive (built-in) types and types that have bindings
 * defined.
 */
...
template <typename T> struct TypeID<std::unordered_map<std::string, T>> {
  static constexpr TYPEID get() { return LightTypeID<val>::get(); }
};

template <typename T> struct TypeID<const std::unordered_map<std::string, T>> {
  static constexpr TYPEID get() { return LightTypeID<val>::get(); }
};
...
template <typename T> struct BindingType<std::unordered_map<std::string, T>> {

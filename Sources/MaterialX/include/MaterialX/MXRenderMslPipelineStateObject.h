...
public:
  ...
  static MslProgramPtr create() { return MslProgramPtr(new MslProgram()); }
  ...
  ///  @param device MetalDevice that  pipeline state object is being created
  ///  on.
  ...
  /// contain some inputs listed on any associated HwShader as those inputs may
  /// have been optimized out if they are unused.
  struct MX_RENDERMSL_API Input {
    ...
    /// HwShader
    string typeString;
    /// Input value. Will only be non-empty if initialized stages with a
    /// HwShader and a value was set during shader generation.
    ...
    Input(int inputLocation, int inputType, int inputSize,
        ...
        : location(inputLocation), resourceType(inputType), size(inputSize),
          isConstant(false), path(inputPath) {}
  };
  ...
  /// An exception is thrown if the parsing of the program for uniforms cannot
  /// be performed.
  ...
  /// The program must have been created successfully first.
  /// An exception is thrown if the parsing of the program for attribute cannot
  /// be performed.
  ...
  /// @param foundList Returned list of found program inputs. Empty if none
  /// found.
  ...
  void bindUniform(const string &name, ConstValuePtr value,
  ...
  /// @param renderCmdEncoder Metal Render Command Encoder that the attribute
  /// being bind to
  ...
  void setEnableMipMaps(bool enableMipMapping) {
    _enableMipMapping = enableMipMapping;
  }
...
protected:
  ...
  ValuePtr findUniformValue(const string &uniformName,

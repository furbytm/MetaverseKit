...
class MenuItem {
public:
  MenuItem(const std::string &name, const std::string &type,
...
// it connects Based off Link struct from ImGui Node Editor
// blueprints-examples.cpp
struct Link {
  int id;
  int _startAttr, _endAttr;
  Link() : _startAttr(-1), _endAttr(-1) {
    static int _id = 0;
    id = ++_id;
  }
};
...
class Graph {
public:
  Graph(const std::string &materialFilename, const std::string &meshFilename,
        const mx::FileSearchPath &searchPath,
        const mx::FilePathVec &libraryFolders, int viewWidth, int viewHeight);

  mx::DocumentPtr loadDocument(mx::FilePath filename);
  void drawGraph(ImVec2 mousePos);

  RenderViewPtr getRenderer() { return _renderer; }
...
private:
  ...
  // from the first node
  ImVec2 layoutPosition(UiNodePtr node, ImVec2 pos, bool initialLayout,
                        int level);
  ...
  void addNode(const std::string &category, const std::string &name,
               const std::string &type);

  void deleteNode(UiNodePtr node);

  // Build the initial graph of a loaded document including shader, material and
  // nodegraph node
  void setUiNodeInfo(UiNodePtr node, const std::string &type,
                     const std::string &category);

  // Check if edge exists in edge vector
  bool edgeExists(UiEdge edge);

  void createEdge(UiNodePtr upNode, UiNodePtr downNode,
  ...
  void updateMaterials(mx::InputPtr input = nullptr,

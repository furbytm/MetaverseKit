...
public:
  UiEdge(UiNodePtr uiDown, UiNodePtr uiUp, mx::InputPtr input)
      : _uiDown(uiDown), _uiUp(uiUp), _input(input) {}
  mx::InputPtr getInput() { return _input; }
  UiNodePtr getDown() { return _uiDown; }
  UiNodePtr getUp() { return _uiUp; }
...
class UiPin {
public:
  UiPin(int id, const char *name, const std::string &type,
        std::shared_ptr<UiNode> node, ed::PinKind kind, mx::InputPtr input,
        mx::OutputPtr output)
      : _pinId(id), _name(name), _type(type), _pinNode(node), _kind(kind),
        _input(input), _output(output), _connected(false) {}
  ...
  const std::vector<UiPinPtr> &getConnections() { return _connections; }
...
public:
  ...
  int getInputConnect() { return _inputNodeNum; }
  int getId() { return _id; }
  const std::vector<UiNodePtr> &getOutputConnections() {
    return _outputConnections;
  }
  mx::NodePtr getNode() { return _currNode; }
  mx::InputPtr getInput() { return _currInput; }
  mx::OutputPtr getOutput() { return _currOutput; }

  void setName(const std::string &newName) { _name = newName; }
  void setPos(ImVec2 pos) { _nodePos = pos; }
  void setInputNodeNum(int num) { _inputNodeNum += num; }
  void setNode(mx::NodePtr node) { _currNode = node; }
  void setInput(mx::InputPtr input) { _currInput = input; }
  void setOutput(mx::OutputPtr output) { _currOutput = output; }
  ...
  mx::NodeGraphPtr getNodeGraph() { return _currNodeGraph; }

  void setNodeGraph(mx::NodeGraphPtr nodeGraph) { _currNodeGraph = nodeGraph; }

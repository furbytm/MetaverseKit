...
MTLComputeEvaluator *MTLComputeEvaluator::Create(
  ...
  if (instance->Compile(srcDesc, dstDesc, duDesc, dvDesc, BufferDescriptor(),
                        BufferDescriptor(), BufferDescriptor(), context))
    return instance;
...
MTLComputeEvaluator *MTLComputeEvaluator::Create(
  ...
  auto instance = new MTLComputeEvaluator();
  if (instance->Compile(srcDesc, dstDesc, duDesc, dvDesc, duuDesc, duvDesc,
                        dvvDesc, context)) {
    return instance;
  }
...
bool MTLComputeEvaluator::Compile(
    BufferDescriptor const &srcDesc, BufferDescriptor const &dstDesc,
    BufferDescriptor const &duDesc, BufferDescriptor const &dvDesc,
    BufferDescriptor const &duuDesc, BufferDescriptor const &duvDesc,
    BufferDescriptor const &dvvDesc, MTLContext *context) {
...
#undef DEFINE
  ...
  auto evalStencilsFunction =
      [_computeLibrary newFunctionWithName:@"eval_stencils"];
  _evalStencils =
  ...
  auto evalPatchesFunction =
      [_computeLibrary newFunctionWithName:@"eval_patches"];
  _evalPatches =
...
bool MTLComputeEvaluator::EvalStencils(
    ...
    id<MTLBuffer> sizesBuffer, id<MTLBuffer> offsetsBuffer,
    id<MTLBuffer> indicesBuffer, id<MTLBuffer> weightsBuffer,
  ...
  return EvalStencils(srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc,
                      dvBuffer, dvDesc, nil, BufferDescriptor(), nil,
                      BufferDescriptor(), nil, BufferDescriptor(), sizesBuffer,
                      offsetsBuffer, indicesBuffer, weightsBuffer,
                      duWeightsBuffer, dvWeightsBuffer, nil, nil, nil, start,
                      end, context);
}
...
bool MTLComputeEvaluator::EvalStencils(
    ...
    id<MTLBuffer> sizesBuffer, id<MTLBuffer> offsetsBuffer,
    id<MTLBuffer> indicesBuffer, id<MTLBuffer> weightsBuffer,
    id<MTLBuffer> duWeightsBuffer, id<MTLBuffer> dvWeightsBuffer,
    id<MTLBuffer> duuWeightsBuffer, id<MTLBuffer> duvWeightsBuffer,
    id<MTLBuffer> dvvWeightsBuffer, int start, int end,
  ...
  [computeEncoder setBuffer:_parameterBuffer
                     offset:0
                    atIndex:PARAMETER_BUFFER_INDEX];
  [computeEncoder setBuffer:sizesBuffer offset:0 atIndex:SIZES_BUFFER_INDEX];
  [computeEncoder setBuffer:weightsBuffer
                     offset:0
                    atIndex:WEIGHTS_BUFFER_INDEX];
  [computeEncoder setBuffer:offsetsBuffer
                     offset:0
                    atIndex:OFFSETS_BUFFER_INDEX];
  [computeEncoder setBuffer:indicesBuffer
                     offset:0
                    atIndex:INDICES_BUFFER_INDEX];
  ...
  if (duWeightsBuffer && dvWeightsBuffer) {
    [computeEncoder setBuffer:duWeightsBuffer
                       offset:0
                      atIndex:DU_WEIGHTS_BUFFER_INDEX];
    [computeEncoder setBuffer:dvWeightsBuffer
                       offset:0
                      atIndex:DV_WEIGHTS_BUFFER_INDEX];
  }
  [computeEncoder setBuffer:duBuffer
                     offset:0
                    atIndex:DU_DERIVATIVE_BUFFER_INDEX];
  [computeEncoder setBuffer:dvBuffer
                     offset:0
                    atIndex:DV_DERIVATIVE_BUFFER_INDEX];
  if (duuWeightsBuffer && duvWeightsBuffer && dvvWeightsBuffer) {
    [computeEncoder setBuffer:duuWeightsBuffer
                       offset:0
                      atIndex:DUU_WEIGHTS_BUFFER_INDEX];
    [computeEncoder setBuffer:duvWeightsBuffer
                       offset:0
                      atIndex:DUV_WEIGHTS_BUFFER_INDEX];
    [computeEncoder setBuffer:dvvWeightsBuffer
                       offset:0
                      atIndex:DVV_WEIGHTS_BUFFER_INDEX];
  }
  if (duuBuffer && duvBuffer && dvvBuffer) {
    [computeEncoder setBuffer:duuBuffer
                       offset:0
                      atIndex:DUU_DERIVATIVE_BUFFER_INDEX];
    [computeEncoder setBuffer:duvBuffer
                       offset:0
                      atIndex:DUV_DERIVATIVE_BUFFER_INDEX];
    [computeEncoder setBuffer:dvvBuffer
                       offset:0
                      atIndex:DVV_DERIVATIVE_BUFFER_INDEX];
  }
  [computeEncoder setComputePipelineState:_evalStencils];

  auto threadgroups =
      MTLSizeMake((count + _workGroupSize - 1) / _workGroupSize, 1, 1);
...
bool MTLComputeEvaluator::EvalPatches(
    ...
    id<MTLBuffer> dvBuffer, const BufferDescriptor &dvDesc, int numPatchCoords,
    id<MTLBuffer> patchCoordsBuffer, const PatchArrayVector &patchArrays,
    id<MTLBuffer> patchIndexBuffer, id<MTLBuffer> patchParamsBuffer,
    MTLContext *context) const {
  return EvalPatches(srcBuffer, srcDesc, dstBuffer, dstDesc, duBuffer, duDesc,
                     dvBuffer, dvDesc, nil, BufferDescriptor(), nil,
                     BufferDescriptor(), nil, BufferDescriptor(),
                     numPatchCoords, patchCoordsBuffer, patchArrays,
                     patchIndexBuffer, patchParamsBuffer, context);
}
...
bool MTLComputeEvaluator::EvalPatches(
    ...
    int numPatchCoords, id<MTLBuffer> patchCoordsBuffer,
    const PatchArrayVector &patchArrays, id<MTLBuffer> patchIndexBuffer,
    id<MTLBuffer> patchParamsBuffer, MTLContext *context) const {
  if (_evalPatches == nil)
    return false;
  ...
  [computeCommandEncoder setBytes:&args
                           length:sizeof(mtl::KernelUniformArgs)
                          atIndex:PARAMETER_BUFFER_INDEX];
  [computeCommandEncoder setBuffer:srcBuffer
                            offset:0
                           atIndex:SRC_VERTEX_BUFFER_INDEX];
  [computeCommandEncoder setBuffer:dstBuffer
                            offset:0
                           atIndex:DST_VERTEX_BUFFER_INDEX];
  if (duBuffer && dvBuffer) {
    [computeCommandEncoder setBuffer:duBuffer
                              offset:0
                             atIndex:DU_DERIVATIVE_BUFFER_INDEX];
    [computeCommandEncoder setBuffer:dvBuffer
                              offset:0
                             atIndex:DV_DERIVATIVE_BUFFER_INDEX];
  }
  if (duuBuffer && duvBuffer && dvvBuffer) {
    [computeCommandEncoder setBuffer:duuBuffer
                              offset:0
                             atIndex:DUU_DERIVATIVE_BUFFER_INDEX];
    [computeCommandEncoder setBuffer:duvBuffer
                              offset:0
                             atIndex:DUV_DERIVATIVE_BUFFER_INDEX];
    [computeCommandEncoder setBuffer:dvvBuffer
                              offset:0
                             atIndex:DVV_DERIVATIVE_BUFFER_INDEX];
  }
  [computeCommandEncoder setBuffer:patchCoordsBuffer
                            offset:0
                           atIndex:PATCH_COORDS_BUFFER_INDEX];
  [computeCommandEncoder setBuffer:patchIndexBuffer
                            offset:0
                           atIndex:PATCH_INDICES_BUFFER_INDEX];
  [computeCommandEncoder setBuffer:patchParamsBuffer
                            offset:0
                           atIndex:PATCH_PARAMS_BUFFER_INDEX];
  assert(!patchArrays.empty());
  [computeCommandEncoder setBytes:&patchArrays[0]
                           length:sizeof(patchArrays[0]) * patchArrays.size()
                          atIndex:PATCH_ARRAYS_BUFFER_INDEX];

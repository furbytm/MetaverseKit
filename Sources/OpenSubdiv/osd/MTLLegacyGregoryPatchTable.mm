...
MTLLegacyGregoryPatchTable::Create(const Far::PatchTable *farPatchTable,
                                   MTLContext *context) {
  ...
  if (!vertexValenceTable.empty()) {
    pt->_vertexValenceBuffer = createBuffer(
        vertexValenceTable.data(),
        vertexValenceTable.size() * sizeof(vertexValenceTable[0]), context);
  }

  if (!quadOffsetsTable.empty()) {
    pt->_quadOffsetsBuffer = createBuffer(
        quadOffsetsTable.data(),
        quadOffsetsTable.size() * sizeof(quadOffsetsTable[0]), context);
  }
  ...
  for (auto i = 0; i < farPatchTable->GetNumPatchArrays(); i++) {
    if (farPatchTable->GetPatchArrayDescriptor(i).GetType() ==
...
void MTLLegacyGregoryPatchTable::UpdateVertexBuffer(id<MTLBuffer> vbo,
                                                    int numVertices,

...
//  2023-04-09: Inputs: Added support for io.AddMouseSourceEvent() to
//  discriminate ImGuiMouseSource_Mouse/ImGuiMouseSource_Pen. 2023-02-01: Fixed
//  scroll wheel scaling for devices emitting events with
//  hasPreciseScrollingDeltas==false (e.g. non-Apple mices). 2022-11-02: Fixed
//  mouse coordinates before clicking the host window. 2022-10-06: Fixed mouse
//  inputs on flipped views. 2022-09-26: Inputs: Renamed ImGuiKey_ModXXX
//  introduced in 1.87 to ImGuiMod_XXX (old names still supported). 2022-05-03:
//  Inputs: Removed ImGui_ImplOSX_HandleEvent() from backend API in favor of
//  backend automatically handling event capture. 2022-04-27: Misc: Store
//  backend data in a per-context struct, allowing to use this backend with
//  multiple contexts. 2022-03-22: Inputs: Monitor NSKeyUp events to catch
//  missing keyUp for key when user press Cmd + key 2022-02-07: Inputs: Forward
//  keyDown/keyUp events to OS when unused by dear imgui. 2022-01-31: Fixed
//  building with old Xcode versions that are missing gamepad features.
...
//  io.AddMouseWheelEvent() API (1.87+). 2022-01-12: Inputs: Added basic
//  Platform IME support, hooking the io.SetPlatformImeDataFn() function.
...
//  2021-12-13: *BREAKING CHANGE* Add NSView parameter to ImGui_ImplOSX_Init().
//  Generally fix keyboard support. Using kVK_* codes for keyboard keys.
//  2021-12-13: Add game controller support.
//  2021-09-21: Use mach_absolute_time as CFAbsoluteTimeGetCurrent can jump
...
static void ImGui_ImplOSX_DestroyBackendData() {
...
bool ImGui_ImplOSX_Init(NSView *view) {
  ...
  io.BackendPlatformName = "Implosx";
  ...
  // be enabled by adding '#define IMGUI_ENABLE_OSX_DEFAULT_CLIPBOARD_FUNCTIONS'
  // in imconfig.h and adding '-framework ApplicationServices' to your linker
  ...
  io.SetPlatformImeDataFn = [](ImGuiViewport *viewport,
                               ImGuiPlatformImeData *data) -> void {
...
#endif
  MAP_ANALOG(ImGuiKey_GamepadLStickLeft, leftThumbstick.xAxis, -thumb_dead_zone,
             -1.0f);
  MAP_ANALOG(ImGuiKey_GamepadLStickRight, leftThumbstick.xAxis,
             +thumb_dead_zone, +1.0f);
  MAP_ANALOG(ImGuiKey_GamepadLStickUp, leftThumbstick.yAxis, +thumb_dead_zone,
             +1.0f);
  MAP_ANALOG(ImGuiKey_GamepadLStickDown, leftThumbstick.yAxis, -thumb_dead_zone,
             -1.0f);
  MAP_ANALOG(ImGuiKey_GamepadRStickLeft, rightThumbstick.xAxis,
             -thumb_dead_zone, -1.0f);
  MAP_ANALOG(ImGuiKey_GamepadRStickRight, rightThumbstick.xAxis,
             +thumb_dead_zone, +1.0f);
  MAP_ANALOG(ImGuiKey_GamepadRStickUp, rightThumbstick.yAxis, +thumb_dead_zone,
             +1.0f);
  MAP_ANALOG(ImGuiKey_GamepadRStickDown, rightThumbstick.yAxis,
             -thumb_dead_zone, -1.0f);
#undef MAP_BUTTON
#undef MAP_ANALOG
...
static ImGuiMouseSource GetMouseSource(NSEvent *event) {
  ...
  // Dear ImGui, which expects absolute touch devices only. There does not seem
...
static bool ImGui_ImplOSX_HandleEvent(NSEvent *event, NSView *view) {
  ...
  if (event.type == NSEventTypeFlagsChanged) {
    ...
    io.AddKeyEvent(ImGuiMod_Shift,
                   (modifier_flags & NSEventModifierFlagShift) != 0);
    ...
    if (key != ImGuiKey_None) {
      ...
      // 'ImGui/mask' is left as a fallback.
...
static void ImGui_ImplOSX_AddTrackingArea(NSView *_Nonnull view) {
  ...
  // controls, not just Dear ImGui widgets. If we had native controls in our
